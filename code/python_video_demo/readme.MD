...
Our demo is a very simple application that will generate a random user name and room, unless you pass them in via GET using respectively the `user` and `room` parameters. We will create two different user types that each have different capabilities. Users can join as a guest that only has the appropriate [permissions](https://docs.signalwire.com/topics/api/video/#signalwire-video-api-room-tokens-create-a-token) to control their own audio/video or as a moderator who has the permissions to control themselves as well as the audio/video of other participants. We will use server side Python and client side Javascript to build an interactive application that can handle calls with multiple users with ease.
...
Our SignalWire Communications API consists of two different APIs that interact to help you build applications.

The server-side API is a collection of REST endpoints used to create and manage rooms, and add access tokens to [them](https://docs.signalwire.com/topics/api/video/#signalwire-video-api).

On the client side, the Javascript SDK allows you to build a custom video experience in a simple, standard-based [way](https://docs.signalwire.com/topics/api/sdk/js/#javascript-sdk).

Documentation is [here](https://docs.signalwire.com/topics/api/)!
...
The server side code will do most of the heavy lifting; within this file you will need functions for handling HTTP requests, creating rooms, and creating tokens. We will also need two flask routes that will handle requests to create a ‘guest token’ and requests to create a ‘moderator token’.

The [create token endpoint](https://docs.signalwire.com/topics/api/video/#signalwire-video-api-room-tokens-create-a-token) will return a JWT token that is used by the front end application to identify what actions you are allowed to perform. When a token is created, the most important required parameter is permissions. Permissions define what each user is allowed to do. For example, if you gave a user no permissions at all, they would be unable to mute or unmute their own audio/video. In this example, we will need a function to create a guest token that will define only the permissions that will allow a user to control their own audio and video. We will also need a function to create a moderator token, which will allow a user to not only control their own audio/video but also to control the audio/video of other individual members, everyone in the room at the same time, or even to kick a user out of the room.

Our first function will define what happens when a token with guest permissions is requested. We will create a dictionary called `payload` with keys `room_name` and `user_name`. Both of these values can be passed through the URL as query params, but if `user` is not specified as a query param, it will be randomized. We also need to create an array that will contain [permissions](https://docs.signalwire.com/topics/api/video/#signalwire-video-api-room-tokens-create-a-token) to control only one's own video/audio. We will add a dictionary key of permissions and set it equal to our permissions array. Next, we will set `result` equal to the result of our function `handle_http()` with the arguments `payload`, which has our room name, user name, and permissions, and `room_tokens`, which is the correct endpoint for creating a token.
...
Our next function will define what happens when a token with moderator permissions is requested. This function is nearly identical to the previous function where we request a token with guest permissions in terms of the structure. The only difference is that we have extra permissions in this function that guests are not permitted. In this demo, as a moderator, you will be able to video mute/ummute other users, kick other users, audio mute/unmute other users, and mute/unmute all members in the room at the same time.
...
Our last function will be responsible for [creating the room](https://docs.signalwire.com/topics/api/video/#signalwire-video-api-rooms-create-room) when called and it's very simple!
...
The `guest.html` and `mod.html` files are quite long so I will not go through them block by block. I will go through the main points of the `<script>` portion instead as you are free to change the layout to anything you'd like. See the `mod.html` or `guest.html` file for the full code.
...
Here you can see that audio and video are both set to true. This is what determines the application will be connecting with both audio and video. If you wanted to create an audio only application (like a clubhouse style app), you could set video here to “false” and you would use the audio track only.
...
The SDK emits events to let the client know about various salient changes (like when a participant joins or leaves or when a participants state changes as in their video is muted or unmuted). You can see an example of how we get the events here:
...
When a room is joined, as long as the user does not already exist, we create an object called `participantListObj` with properties `userId` and `userName`. We will use `params.room.members[participant]` to get the userID and userName for these properties. We will create another object `partObj` where the key is `participantListObj.userId` and is equal to the object `participantListObj`.
...
Again, the code is nearly exactly the same when members join or leave except that there is only one member in `params.room.member` so the code would instead look like this instead:
...
When you are in a room, it is helpful to be able to see all of the users who are in the room with you. I will define a function `listParticipants()` that will list all the current users and if you're a moderator, allow you to control them with buttons. `listParticipants()` will be called any time the room is joined and any time a member joins/leaves so that it is always updated.

![Participant List](assets/listed_participants.png)
...
In the moderator view, we have several functions for controlling other users shown below. These will take a member ID (gathered from `room.params` on room state changes) as the argument and contain the methods shown in the [documentation](https://docs.signalwire.com/topics/api/sdk/js/#javascript-sdk-signalwire-video-createroomobject-methods) for muting/unmuting/kicking a user. These will not work if the token does not have the correct permission defined. These will be attached as on click events to buttons that are next to each user on the nav bar **only if** you are in the moderator view.
